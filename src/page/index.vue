        ...
        <section class="hgf_df_drtd">
            ...
            <section class="d_kj_derrts ">
                ...
                <section class="hao_ddf_er">
                    <img :src="sd.header_image" class="yj user_icions">
                    <p class="fz12">
                        {{sd.user_name}}
                    </p>
                    <section class="z9 fz14 mt5" v-if="sd.zige==1&&sd.need_people!=0">
                       <i class="f_i dpoi_der"></i> 您的礼包还剩 <span class="fz20 red">{{sd.need_people}}</span>  层要拆哦！
                         <i class="f_i dpoi_der ab"></i>
                    </section>
                    <p class="mt20">
    ...
    <span class="yj4 cf biank_df">{{is_drte_er.h}}</span>
         ...
         <span class="yj4 cf biank_df">{{is_drte_er.h_m}}</span>
                毫秒
                    ...
                    <section v-if="sd.need_people!=0">
                       <button class="sd_jh_deert" @click="is_Ddffe=true" v-if="sd.zige==0">获取礼包</button>
                       <button class="sd_jh_deert" v-else  @click="is_sd_fn=true">邀请好友拆礼包</button>
                        </section>

                      <section v-if="sd.need_people==0">
                          <button class="sd_jh_deert"   @click="is_zhifud=true" v-if="sd.is_payed==0">立即支付</button>
                           <button class="sd_jh_deert"    v-if="sd.is_payed==1">已支付</button>
                     </section>
...
<!--
                <span class="s_kjh_de">1</span>礼包共有10层，分享给10名好友帮自己拆礼包，即可享受超低价购买大礼包的优惠；<br>
                        <span class="s_kjh_de">2</span> 每个好友只能帮拆1层礼包；<br>
                        <span class="s_kjh_de">3</span> 礼包每人限购1个；<br>
                        <span class="s_kjh_de">4</span> 将本活动链接转发给您的朋友A，只要A购买，您即可获得X.XX元现金红包；<br>
                        <span class="s_kjh_de">5</span> 您的朋友A再次分享给朋友B，B购买，您可再次获得X.XX元现金红包。
-->
    ...
    <section class="hgf_df_drtd ab">
        ...
        <section class="pr sdf_jhhg_wer bgff">
          ...
          <section class="hgf_df_drtd">
            <section class="d_kj_derrts ">
                <section class="sd_lih_yer pr ac pt20">
                     <i class="f_i tie_derttc">好友拆礼包榜</i>

                    <vue-seamless-scroll :data="sd.liebian_list" :class-option="optionSingleHeightTime" class="seamless-warp fdg_kjhj_deeer" v-if="sd.liebian_list.length>0">

                    <ul class=" "  >
                <li class="mui-row pt10 pm10 bbm" v-for="(sf,idx) in sd.liebian_list">
                    <section class="mui-col-xs-6 dian">
                        <img :src="sf.friend_header_image" class="user_icon_ser yj">

                        <section class="ov dian pl10">
                            <p class="z3 fz16">{{sf.friend_name}}</p>
                            <p class="fz12 z9">{{sf.msg}}</p>
                        </section>
                        <p class="qc"></p>
                    </section>

                      <section class="mui-col-xs-6 fz12 pr">
                         <span class="cz f_b mt10"> 帮您拆掉了第<i class="red fz16 b">{{sf.id}}</i> 层礼包</span>
                          <i class="f_i libao_eerr"></i>
                    ...
                    <section v-else class="cen z9 fz14 pt40 pm40">
                    “您的礼包还没有好友帮拆哦”
                    </section>
    </section>
...
<!--        头部的滚动条-->
  ...
  <source :src="sd.bgmusic" />
    ...
    <name_page v-if="is_Ddffe" @ssd_Sdr="sd.zige=1;is_Ddffe=false" :user_name="sd.user_name" :mobile_phone="sd.mobile_phone" :goods_id="goods_id" :friend_header_image="sd.header_image" ></name_page>
<!--    支付的弹窗-->
  <payment_page :is_drte_er="is_drte_er"  @ssd_Sdr="is_zhifud=false" :is_ssd="sd.gift_bag_limit_time" v-if="is_zhifud&&sd.is_payed==0" :friend_header_image="sd.header_image" :friend_name="sd.user_name" :jiage="sd.shop_price" :goods_id="goods_id" :shop_id="sd.shop_id"></payment_page>
...
<!--
   <payment_page :is_drte_er="is_drte_er"  @ssd_Sdr="is_zhifud=false" :is_ssd="sd.gift_bag_limit_time" v-if="true" :friend_header_image="sd.header_image" :friend_name="sd.user_name" :jiage="sd.shop_price" :goods_id="goods_id" :shop_id="sd.shop_id"></payment_page>

-->

<!--    帮ta拆红包-->
    <cailibao v-if="is_shareid" @ssd_Sdr="is_shareid=false"  @ckasd="ckasd" :user_name="sd.user_name" :friend_header_image="sd.header_image" :friend_name="sd.user_name" :goods_id="goods_id" :shareid="shareid"></cailibao>
    </div>
        </template>
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getdate(df_der) {
                ...
                if (!shareid) {
                    try {
                        shareid = window.location.search.split('shareid=')[1].split("&")[0]
                    } catch (e) {
                ...
                this.post("liebian/home", home, function(data) {
                    ...
                    th.fenxiang(data.activity_title, fx_url, data.goods_thumb, "呼朋唤友拆礼包")

                    if (data.need_people == 0) {
                        th.is_zhifud = true
                    }
                    ...
                    if (data.shareid) {
                        th.hf(`share_fx?goods_id=${th.goods_id}&shareid=${data.shareid}`)
                    } else {
                        th.is_sddf = true
                    }
